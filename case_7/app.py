...
def load_data_from_gdrive(file_id):
    ...
    head = file_bytes.getvalue()[:100].decode('utf-8', errors='ignore').lower()
    if '<html' in head:
        raise Exception("Получен HTML вместо CSV. Файл слишком большой или требуется дополнительное подтверждение загрузки.")
