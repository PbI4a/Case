...
st.title("🛒 Amazon US Product Reviews")
...
with tabs[2]:
    ...
    fig1 = px.bar(
        grp, x='year_month', y='reviews',
        title="Отзывы по месяцам"
    )
    fig2 = px.line(
        grp, x='year_month', y='avg_rating',
...
with tabs[3]:
    ...
    fig = px.line(
        comp, x='year_month', y='star_rating',
        color='product_title',
...
with tabs[4]:
    ...
    sel_s = st.selectbox(
        "Тональность для облака",
        ['positive', 'neutral', 'negative']
    )
    txt = " ".join(
        df_filtered[df_filtered['sentiment_simple'] == sel_s]['review_body']
        .dropna()
    )
    ...
    tokens = pd.Series(
        " ".join(df_filtered['review_body'].dropna()).lower().split()
    )
...
with tabs[7]:
    ...
    page_size = st.number_input(
        "Отзывы на страницу", min_value=5, max_value=50, value=10, step=5
    )
    ...
    subset = df_filtered.iloc[start:start + page_size][
